...
# Here we inject our custom plugin into the build.
COPY plugins/ /build/mosq/plugins/
...
RUN set -x && \
    ...
    install -s -m755 /build/mosq/plugins/force-retain/mosquitto_force_retain.so /usr/lib/mosquitto_force_retain.so && \
